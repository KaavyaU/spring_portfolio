...
public class ImageInfo {
    ...
    public String asciiArt() {
        ...
        try {
            BufferedImage img = ImageIO.read(new URL(url)); // Saving internet image to BufferedImage
            double [][] grayscaleVals = imageToGrayscale(img);
            for (int i = 0; i < grayscaleVals.length; i++){
                for (int j = 0; j < grayscaleVals[0].length; j++){
                    for (int k=0; k < GRAYSCALE_VALUES.length; k++){
                        if (grayscaleVals[i][j] <= GRAYSCALE_VALUES[k]){
                            asciiChars += GRAYSCALE_CHARS.charAt(k);
                            break;
                        }
                    }
                }
            ...
            //byte[] pixels = image_to_pixels(img); // See method definition
            //int[] pixels_int = asciiArt(pixels);
            //String base64 = pixels_to_base64(img.getWidth(), img.getHeight(), pixels_int);
           // return "data:image/jpg;base64,"+base64;
            FileWriter asciiFile = new FileWriter(file);
            asciiFile.write(asciiChars);
            asciiFile.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    /*
    public int[] asciiArt(byte[] pixels){
    ...
    public double[][] imageToGrayscale(BufferedImage img){
        width  = img.getWidth();
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                ...
                image[i][j] = getBrightness(new Color(rgb));
            }
        }
        return image;
    }
